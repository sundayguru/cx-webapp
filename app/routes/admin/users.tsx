...
export const action = async ({ request }: Route.ActionArgs) => {
  ...
  if (intent === 'toggleDeactivate') {
    ...
    return data({ success: true, message: `User ${!isDeactivated ? 'deactivated' : 'activated'}` });
  }
  ...
  if (intent === 'toggleBan') {
    ...
    return data({ success: true, message: `User ${!isBanned ? 'banned' : 'unbanned'}` });
  }
...
export default function AdminUsersPage({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className='mx-auto max-w-6xl px-4 py-8 relative flex-1'>
      <div className='mb-8'>
                  ...
                  <td className='px-6 py-4'>
                    <div className='flex items-center justify-end gap-2'>
                      <fetcher.Form method='post'>
                        ...
                        <input type='hidden' name='isDeactivated' value={String(u.isDeactivated || false)} />
                        <button
                          type='submit'
                          className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-semibold transition-colors ${u.isDeactivated
                      ...
                      <fetcher.Form method='post'>
                        ...
                        <input type='hidden' name='userId' value={u.id} />
                        <input type='hidden' name='isBanned' value={String(u.isBanned || false)} />
                        <button
                          type='submit'
                          className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-semibold transition-colors ${u.isBanned
                            ? 'bg-[#5A5A40] text-white hover:bg-[#4a4a35]'
                            : 'bg-red-100 text-red-700 hover:bg-red-200'
                            }`}
                        >
                          {u.isBanned ? <Shield size={14} /> : <ShieldAlert size={14} />}
