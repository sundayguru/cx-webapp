...
import { Edit3, FileText, X, ExternalLink } from 'lucide-react';
import {
...
const PdfModal = ({
  ...
  const pdfUrl = `/api/course/serve/${contentKey}`;
        ...
        <div className='fixed inset-0 z-[100] flex items-center justify-center p-0 md:p-8'>
          ...
          >
            <div className='flex items-center justify-between border-b border-black/5 bg-white p-4 md:p-6'>
              <div className='flex items-center gap-3 md:gap-4 text-[#5A5A40]'>
                <div className='flex h-9 w-9 md:h-10 md:w-10 items-center justify-center rounded-xl bg-[#5A5A40]/10'>
                  <FileText size={18} className='md:size-5' />
                </div>
                <div className='min-w-0'>
                  <h3 className='leading-tight font-bold text-[#1a1a1a] truncate'>
                    Course Content
                  </h3>
                  <p className='text-[10px] md:text-xs font-bold tracking-widest text-black/40 uppercase'>
                    {code}
                  </p>
                </div>
              </div>
              <div className='flex items-center gap-2'>
                <a
                  ...
                  className='flex h-9 w-9 md:h-10 md:w-10 items-center justify-center rounded-full bg-black/5 transition-colors hover:bg-black/10'
            ...
            <div className='flex-1 bg-gray-100 p-2 md:p-4'>
              <iframe
                src={pdfUrl}
                className='h-full w-full rounded-xl md:rounded-2xl border border-black/5 bg-white'
                title={title}
              />
            </div>
            {/* Mobile help text */}
            <div className='md:hidden border-t border-black/5 bg-white px-4 py-3 text-center'>
