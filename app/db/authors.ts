...
export const getAuthorById = async (id: string) => {
  const db = getDb();
  const results = await db.select().from(authors).where(eq(authors.id, id)).limit(1);
  return results[0] || null;
};

export const getAuthorBySlug = async (slug: string) => {
  const db = getDb();
  const results = await db.select().from(authors).where(eq(authors.slug, slug)).limit(1);
  return results[0] || null;
};
