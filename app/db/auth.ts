...
import { users, accounts, sessions, verificationTokens, passwordResetTokens, type InsertUser, type InsertAccount, type InsertSession } from './schemas';
import { hashPassword, verifyPassword, generateResetToken, getSessionExpiry } from '~/utils/auth.server';
import { v4 as uuidv4 } from 'uuid';
import { logError } from '~/utils/logger';

// User operations
export const createUser = async (userData: Omit<InsertUser, 'createdAt' | 'updatedAt'> & { password?: string }): Promise<InsertUser | null> => {
    try {
        const db = getDb();
        const userWithId = {
            ...userData,
...
export const createUserWithPassword = async (email: string, password: string, firstName: string, lastName: string): Promise<InsertUser | null> => {
    try {
...
export const createAccount = async (accountData: InsertAccount): Promise<void> => {
    try {
        const db = getDb();
...
export const getUserByAccount = async (provider: string, providerAccountId: string) => {
    try {
        const db = getDb();
        const [account] = await db.select()
            .from(accounts)
...
// Session operations
export const createSession = async (sessionData: InsertSession): Promise<void> => {
    try {
        const db = getDb();
...
export const createVerificationToken = async (identifier: string, token: string, expires: string): Promise<void> => {
    try {
        const db = getDb();
...
export const createPasswordResetToken = async (userId: string, token: string, expires: string): Promise<void> => {
    try {
        const db = getDb();
...
export const generatePasswordResetTokenForEmail = async (email: string): Promise<{ token: string; expires: string } | null> => {
    const user = await getUserByEmail(email);
    if (!user) return null;
