...
export const AuthorSelect = ({ value, label, onChange, error }: AuthorSelectProps) => {
                  ...
                  {authors.map((author) => (
                    ...
                    >
                      <div className={`flex h-8 w-8 items-center justify-center rounded-lg ${
                        value === author.id ? 'bg-white/20' : 'bg-[#5A5A40]/10'
                      }`}>
                        <UserCircle size={16} className={value === author.id ? 'text-white' : 'text-[#5A5A40]'} />
                      </div>
                      <div className="flex-1 overflow-hidden">
                        <p className="truncate font-medium">{author.name}</p>
                        <p className={`truncate text-xs ${value === author.id ? 'text-white/60' : 'text-black/40'}`}>
                          @{author.slug}
                        </p>
                      </div>
                ...
                <div className="mt-2 border-t border-black/5 pt-2">
                  ...
                  >
                    ...
                    <div className="flex-1 overflow-hidden">
                      ...
                      <p className="text-xs text-black/40">Add this author to our database</p>
                    </div>
                  </button>
                </div>
              )}
